...
export function ColorTool() {
  const l = useLocalized(convertersL).color
  ...
  return (
    <div className="space-y-4">
      ...
      {!parsed && input.trim() && <ErrorNote msg={l.invalid} />}
      {parsed && (
        <>
...
export function ImgBase64Tool() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <div className="text-[11px] text-muted uppercase tracking-wider mb-2">{l.step1}</div>
        <input
          ref={fileRef}
          type="file"
          accept="image/*"
      ...
      <div className="border-t border-line-soft pt-4">
        ...
        {decodePreview && (
          <div className="mt-2 flex items-start gap-3">
            ...
            <Btn onClick={download}>{l.download}</Btn>
          </div>
        )}
        {decodeInput.trim() && !decodePreview && <ErrorNote msg={l.invalid} />}
      </div>
    </div>
  )
}
...
export function StringEscapeTool() {
  ...
  const names: Record<string, string> = {
    url: l.urlName,
    doubleUrl: l.doubleUrlName,
    html: l.htmlName,
    unicode: l.unicodeName,
    hex: l.hexName,
    base64: l.base64Name,
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <ErrorNote msg={err} />
      <TA value={output} readOnly label={l.outputLabel} rows={4} />
